class UserController < ApplicationController
  ...
  def save_article
    ...
    current_user.save_article(params[:article_id], params[:article_title])
    if (params[:js] == 1)
      render :nothing => true
    else
  ...
  def unsave_article
    ...
    current_user.unsave_article(params[:article_id])
    if (params[:js] == 1)
      render :nothing => true
    else
      redirect_to saved_articles_path
  ...
  def delete_article
    ...
    current_user.delete_article(params[:article_id])
  ...
  def preference
    @saved_preference = current_user.get_preference()
		render 'preference'
  end

  def add_preference
    current_user.add_preference(params[:preference])
